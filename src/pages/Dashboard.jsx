    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-1 font-mono uppercase tracking-widest text-[10px]">Real-time telemetry and incident reports</p>
        </div>
        <div className="flex gap-3">
            <div className="px-4 py-2 bg-white/5 border border-white/10 rounded-xl flex items-center gap-2 shadow-xl">
        ...
        {stats.map((stat, i) => (
          ...
          >
            <div className="flex items-start justify-between">
              ...
              <div className="flex items-center gap-1 text-[10px] font-mono font-bold text-slate-400 uppercase tracking-tighter">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Incident Feed */}
        <div className="lg:col-span-2 glass-card overflow-hidden self-start">
          <div className="p-6 border-b border-white/10 flex items-center justify-between bg-white/5">
            <h3 className="text-xs font-bold text-white uppercase tracking-[0.2em]">Priority Incident Feed</h3>
            <div className="flex items-center gap-2">
               <span className="px-3 py-1 bg-white/10 border border-white/10 rounded-lg text-[9px] font-bold text-slate-400 uppercase tracking-widest">
          ...
          <div className="p-0">
             {incidents.length === 0 ? (
               <div className="p-12 flex flex-col items-center justify-center text-center">
                  ...
                  <p className="text-slate-400 text-xs mt-2 font-mono">No pending emergencies detected in current sector.</p>
               </div>
             ) : (
               incidents.map((inc, i) => (
                  <motion.div 
                  ...
                  >
                   <div className="flex gap-6">
                       <div className="flex flex-col items-center">
                          <span className="text-[10px] font-mono text-emerald-500 font-bold">14:2{i}</span>
                      ...
                      <div className="flex-1">
                         <div className="flex items-center gap-3 mb-2">
                             <span className={`px-2 py-0.5 rounded text-[8px] font-bold border uppercase ${
                               inc.severity === 'high' ? 'bg-rose-500/10 text-rose-500 border-rose-500/20' : 
                          ...
                          <h4 className="text-sm font-bold text-white group-hover:text-emerald-400 transition-colors uppercase">{inc.type} - Alpha Sector</h4>
                          <p className="text-xs text-slate-400 mt-1 leading-relaxed">Incident detected at coordinates [{inc.lat.toFixed(4)}, {inc.lng.toFixed(4)}]. Immediate response required.</p>
                            ...
                            {inc.status === 'detected' ? (
                              ...
                              >
                               ...
                               <button 
                                ...
                                className="flex items-center gap-2 bg-white/10 hover:bg-white/20 px-4 py-2 rounded-lg text-white font-bold text-[10px] uppercase tracking-widest transition-all shadow-lg border border-white/10"
                              >
                ...
                {[
                  ...
                  <div key={asset.label} className="space-y-2">
                    <div className="flex justify-between items-end">
                      <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{asset.label}</span>
